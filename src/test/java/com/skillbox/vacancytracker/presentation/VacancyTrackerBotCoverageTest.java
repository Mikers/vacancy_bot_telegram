...
class VacancyTrackerBotCoverageTest {
    ...
    @Test
    void shouldGetBotName() {
        String name = bot.getBotName();
        assertThat(name).isEqualTo("test-bot");
    }
    
    @Test
    void shouldHandleExecuteMethodException() throws TelegramApiException {
        ...
        when(commandDispatcher.dispatch(any(UserMessage.class))).thenReturn(response);
        doThrow(new TelegramApiException("Telegram API error")).when(telegramClient).execute(any(SendMessage.class));
    ...
    @Test
    void shouldHandleErrorWithCallbackQuery() {
        Update update = mock(Update.class);
    ...
    @Test
    void shouldHandleSecondaryExceptionInErrorHandling() throws TelegramApiException {
        Update update = mock(Update.class);
        var message = mock(org.telegram.telegrambots.meta.api.objects.message.Message.class);
        var user = mock(org.telegram.telegrambots.meta.api.objects.User.class);
        ...
        when(commandDispatcher.dispatch(any(UserMessage.class))).thenThrow(new RuntimeException("Primary error"));
        doThrow(new TelegramApiException("Secondary error")).when(telegramClient).execute(any(SendMessage.class));
